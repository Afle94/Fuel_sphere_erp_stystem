<?php

namespace App\Exports;

use App\Exports\Concerns\AppliesReportHeading;
use App\Models\AccountName;
use Maatwebsite\Excel\Concerns\FromCollection;
use Maatwebsite\Excel\Concerns\ShouldAutoSize;
use Maatwebsite\Excel\Concerns\WithCustomStartCell;
use Maatwebsite\Excel\Concerns\WithHeadings;
use Maatwebsite\Excel\Concerns\WithMapping;
use Maatwebsite\Excel\Concerns\WithStyles;
use Maatwebsite\Excel\Concerns\WithTitle;
use PhpOffice\PhpSpreadsheet\Style\Alignment;
use PhpOffice\PhpSpreadsheet\Style\Border;
use PhpOffice\PhpSpreadsheet\Style\Fill;
use PhpOffice\PhpSpreadsheet\Worksheet\Worksheet;

class AccountMasterExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithCustomStartCell, ShouldAutoSize
{
    use AppliesReportHeading;

    public function __construct(private array $theme = [])
    {
        $this->theme = array_merge([
            'primary' => '#0f766e',
            'primaryDark' => '#115e59',
            'accent' => '#f59e0b',
            'bgEnd' => '#eef5f3',
        ], $theme);
    }

    public function collection()
    {
        return AccountName::orderBy('account_perticular')->get();
    }

    public function title(): string
    {
        return 'Account Master';
    }

    public function headings(): array
    {
        return [
            'ID',
            'Account Name',
            'Under Group',
            'Opening Balance',
            'Type',
            'Address',
            'City / District',
            'State',
            'Email',
            'Mobile',
            'Phone',
            'GST No.',
            'Created Date',
            'Updated Date',
        ];
    }

    public function map($account): array
    {
        return [
            $account->id,
            $account->account_perticular,
            $account->under_group,
            (float) $account->opening_balance,
            $account->transaction_type,
            $account->address ?: '-',
            $account->city ?: '-',
            $account->state ?: '-',
            $account->email ?: '-',
            $account->mobile_number ?: '-',
            $account->phone_number ?: '-',
            $account->gst_number ?: '-',
            optional($account->created_at)->format('d M Y') ?: '-',
            optional($account->updated_at)->format('d M Y') ?: '-',
        ];
    }

    public function styles(Worksheet $sheet): array
    {
        $highestRow = $sheet->getHighestRow();
        $highestColumn = $sheet->getHighestColumn();
        $primary = ltrim($this->theme['primary'], '#');
        $primaryDark = ltrim($this->theme['primaryDark'], '#');
        $bgEnd = ltrim($this->theme['bgEnd'], '#');

        $this->applyReportHeading($sheet, $highestColumn, $highestRow);

        $sheet->getStyle("A4:{$highestColumn}4")->applyFromArray([
            'font' => [
                'bold' => true,
                'color' => ['rgb' => 'FFFFFF'],
                'size' => 11,
            ],
            'fill' => [
                'fillType' => Fill::FILL_SOLID,
                'startColor' => ['rgb' => strtoupper($primary)],
            ],
            'alignment' => [
                'horizontal' => Alignment::HORIZONTAL_CENTER,
                'vertical' => Alignment::VERTICAL_CENTER,
            ],
        ]);

        $sheet->getStyle("A4:{$highestColumn}{$highestRow}")->applyFromArray([
            'borders' => [
                'allBorders' => [
                    'borderStyle' => Border::BORDER_THIN,
                    'color' => ['rgb' => strtoupper($primaryDark)],
                ],
            ],
            'alignment' => [
                'vertical' => Alignment::VERTICAL_TOP,
                'wrapText' => true,
            ],
        ]);

        if ($highestRow >= 5) {
            $sheet->getStyle("A5:A{$highestRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle("D5:D{$highestRow}")->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->getStyle("D5:D{$highestRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);

            for ($row = 5; $row <= $highestRow; $row++) {
                if ($row % 2 === 0) {
                    $sheet->getStyle("A{$row}:{$highestColumn}{$row}")->getFill()
                        ->setFillType(Fill::FILL_SOLID)
                        ->getStartColor()
                        ->setRGB(strtoupper($bgEnd));
                }
            }
        }

        $sheet->getRowDimension(4)->setRowHeight(24);
        $sheet->freezePane('A5');

        return [];
    }
}



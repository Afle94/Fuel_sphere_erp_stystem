...
class ChequePaymentExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithCustomStartCell, ShouldAutoSize
{
    use AppliesReportHeading;

    public function __construct(private Collection $chequepayments, private string $selectedDate, private array $theme = [])
    {
    ...
    {
        return ['Date', 'Slip No.', 'Cheque No.', 'Cheque Date', 'Credit', 'Debit', 'Amount', 'Narration'];
    }

    public function map($payment): array
    {
        return [
            substr((string) $payment->date, 0, 10) ?: $this->selectedDate,
            $payment->slip_no,
            $payment->cheque_no,
            substr((string) $payment->cheque_date, 0, 10),
            $payment->credit,
            $payment->debit,

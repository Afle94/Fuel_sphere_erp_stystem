...
class RegisterCashReceiptController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        ...
        if ($entries->isEmpty()) {
            return redirect()
                ->route('RegisterCashReceiptFilter', $request->query())
                ->with(
                    'error',
                    'No cash receipt register entries available to export.'
                );
        }
        ...
        if (! $this->shouldStreamRawPdf($request)) {
            return $this->pdfViewer($request, 'Cash Receipt Register');
        }

        $html = view(
            'Cash_Receipt_Register_pdf',
            compact(
                'entries',
                'theme',
                'periodLabel',
        ...
        return response(
            ...
            ->header(
                'Content-Disposition',
                'inline; filename="CashReceiptRegister.pdf"'
            );
    }

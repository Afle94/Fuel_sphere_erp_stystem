...
class RegisterChequePaymentController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        ...
        if ($entries->isEmpty()) {
            return redirect()
                ->route('RegisterChequePaymentFilter', $request->query())
                ->with(
                    'error',
                    'No cheque payment register entries available to export.'
                );
        }
        ...
        if (! $this->shouldStreamRawPdf($request)) {
            return $this->pdfViewer($request, 'Cheque Payment Register');
        }

        $html = view(
            'Cheque_Payment_Register_pdf',
            compact(
                'entries',
                'theme',
                'periodLabel',
        ...
        return response(
            ...
            ->header(
                'Content-Disposition',
                'inline; filename="ChequePaymentRegister.pdf"'
            );
    }
    ...
    public function excel(Request $request)
    {
        ...
        return Excel::download(
            new ChequePaymentExport(
                $entries,
                $this->periodLabel($request),
                $this->exportTheme($request)
            ),
            'ChequePaymentRegister.xlsx'
        );
    }

...
class RegisterCashPaymentController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        ...
        if ($entries->isEmpty()) {
            return redirect()
                ->route('RegisterCashPaymentFilter', $request->query())
                ->with(
                    'error',
                    'No cash payment register entries available to export.'
                );
        }
        ...
        if (! $this->shouldStreamRawPdf($request)) {
            return $this->pdfViewer($request, 'Cash Payment Register');
        }

        $html = view(
            'Cash_Payment_Register_pdf',
            compact(
                'entries',
                'theme',
                'periodLabel',
        ...
        return response(
            ...
            ->header(
                'Content-Disposition',
                'inline; filename="CashPaymentRegister.pdf"'
            );
    }

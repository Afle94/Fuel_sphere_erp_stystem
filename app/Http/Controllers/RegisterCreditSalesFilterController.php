...
use App\Exports\CreditSalesRegisterExport;
use App\Models\CreditSales;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;
use Mpdf\Mpdf;

class RegisterCreditSalesFilterController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        ...
        if ($entries->isEmpty()) {
            ...
            return redirect()
                ->route('creditsalesregisterfilter', $request->query())
                ->with(
                    'error',
                    'No credit sales register entries available to export.'
                );
        }

        if (! $this->shouldStreamRawPdf($request)) {

            return $this->pdfViewer($request, 'Credit Sales Register');
        }
        ...
        return response(
            ...
            ->header(
                'Content-Disposition',
                'inline; filename="CreditSalesRegister.pdf"'
            );
    }
    ...
    public function excel(Request $request)
    {
        ...
        return Excel::download(
            new CreditSalesRegisterExport(
                $entries,
                $this->periodLabel($request),
                $search,
                $this->exportTheme($request)
            ),
            'CreditSalesRegister.xlsx'
        );
    }

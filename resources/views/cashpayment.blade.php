...
<head>
    ...
    <title>Cash Payment | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cash-payment-workspace.app-shell-with-sidebar {
        ...
        .cash-payment-workspace.app-shell-with-sidebar.menu-collapsed {
...
<body>
    @php
        $cashpayments = collect($cashpayments ?? $cashpayment ?? []);
        $totalAmount = $cashpayments->sum(fn ($payment) => (float) ($payment->amount ?? 0));
        ...
        $storeUrl = \Illuminate\Support\Facades\Route::has('cashpayment.store') ? route('cashpayment.store') : route('cashpayment');
        $listUrl = \Illuminate\Support\Facades\Route::has('cashpayment') ? route('cashpayment') : url('/cashpayment');
        $hasUpdateRoute = \Illuminate\Support\Facades\Route::has('cashpayment.update');
        $hasDestroyRoute = \Illuminate\Support\Facades\Route::has('cashpayment.destroy');
        $hasPdfRoute = \Illuminate\Support\Facades\Route::has('cashpayment.pdf');
        $hasExcelRoute = \Illuminate\Support\Facades\Route::has('cashpayment.excel');
    @endphp
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Cash Payment</div>
    ...
    <div class="app-shell-with-sidebar cash-payment-workspace" id="dashboardPage">
        @include('partials.fueltracker-menu')

        <main class="cash-payment-page">
            <section class="page-title" aria-labelledby="cashPaymentTitle">
                <div>
                    <p class="eyebrow">Transactions</p>
                    <h1 id="cashPaymentTitle">Cash Payment</h1>
                </div>
                <span class="record-count">{{ $cashpayments->count() }} {{ $cashpayments->count() === 1 ? 'entry' : 'entries' }}</span>
            </section>

            <div class="content-grid">
                <section class="panel form-panel" aria-labelledby="cashPaymentEntryTitle">
                    <div class="panel-head">
                        <h2 id="cashPaymentEntryTitle">Cash Payment Entry</h2>
                    </div>
                    ...
                    <form class="payment-form" id="cashPaymentForm" method="POST" action="{{ $storeUrl }}" data-store-url="{{ $storeUrl }}" autocomplete="off">
                        @csrf
                        <input type="hidden" name="_method" id="cashPaymentFormMethod" value="PUT" disabled>

                        <div class="form-grid">
                            <div class="field">
                                <label for="paymentDate">Date</label>
                                <input id="paymentDate" type="date" name="date" value="{{ old('date', $today) }}">
                            </div>
                            ...
                            <div class="field">
                                ...
                                <div class="party-dropdown" id="creditDropdown" data-payment-dropdown>
                                    ...
                                    <button type="button" class="party-dropdown-button" aria-haspopup="listbox" aria-expanded="false">
                                        <span class="party-dropdown-text" data-payment-dropdown-text>{{ $selectedCredit ?: 'Select credit' }}</span>
                            ...
                            <div class="field">
                                ...
                                <div class="party-dropdown" id="debitDropdown" data-payment-dropdown>
                                    <input type="text" class="party-dropdown-value" id="debit" name="debit" value="{{ $selectedDebit }}" autocomplete="off">
                                    <button type="button" class="party-dropdown-button" aria-haspopup="listbox" aria-expanded="false">
                                        <span class="party-dropdown-text" data-payment-dropdown-text>{{ $selectedDebit ?: 'Select debit' }}</span>
                                        <span class="party-dropdown-arrow" aria-hidden="true"></span>
                                    </button>
                ...
                <section class="panel list-panel" aria-labelledby="cashPaymentListTitle">
                    <div class="table-toolbar">
                        <div class="toolbar-title" id="cashPaymentListTitle">Cash Payment List</div>
                        <div class="toolbar-actions">
                            @if ($cashpayments->isNotEmpty() && ($hasPdfRoute || $hasExcelRoute))
                                <div class="export-actions" aria-label="Cash payment export actions">
                                    @if ($hasPdfRoute)
                                        <a href="{{ route('cashpayment.pdf', ['date' => $today]) }}" class="export-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                                    @endif
                                    @if ($hasExcelRoute)
                                        <a href="{{ route('cashpayment.excel', ['date' => $today]) }}" class="export-btn" data-themed-export>Excel</a>
                                    @endif
                                </div>
                            @endif
                            ...
                            <tbody>
                                @forelse ($cashpayments as $payment)
                                    <tr>
                                        ...
                                        <td>{{ $payment->debit }}</td>
                                        <td class="number-cell">{{ number_format((float) $payment->amount, 2) }}</td>
                                            ...
                                            <div class="row-actions">
                                                @if ($hasUpdateRoute)
                                                    <button
                                                        ...
                                                        class="update-btn"
                                                        data-edit-payment
                                                        data-update-url="{{ route('cashpayment.update', $payment) }}"
                                                        data-date="{{ optional($payment->date)->format('Y-m-d') }}"
                                                        data-slip-no="{{ $payment->slip_no }}"
                                                        data-credit="{{ $payment->credit }}"
                                                        data-debit="{{ $payment->debit }}"
                                                        data-amount="{{ $payment->amount }}"
                                                        data-narration="{{ $payment->Narration }}"
                                                    >
                                                        Update
                                                    </button>
                                                @endif
                                                ...
                                                @if ($hasDestroyRoute)
                                                    <form method="POST" action="{{ route('cashpayment.destroy', $payment) }}" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="delete-btn" data-delete-payment="Slip {{ $payment->slip_no }}">Delete</button>
                                                    </form>
                                                @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="empty-state">No cash payment entries found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div class="delete-modal" id="deleteConfirmModal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-hidden="true">
        <div class="delete-dialog">
            ...
            <p class="delete-dialog-body">Are you sure you want to delete <strong id="deletePaymentName">this entry</strong>? This action cannot be undone.</p>
    ...
    <script>
        ...
        const formMethodInput = document.getElementById('cashPaymentFormMethod');
        ...
        const editButtons = document.querySelectorAll('[data-edit-payment]');
        const cashPaymentListUrl = @json($listUrl);
        const deleteModal = document.getElementById('deleteConfirmModal');
        const deletePaymentName = document.getElementById('deletePaymentName');
        const deleteCancelBtn = document.getElementById('deleteCancelBtn');
        const deleteConfirmBtn = document.getElementById('deleteConfirmBtn');
        const dropdowns = Array.from(document.querySelectorAll('[data-payment-dropdown]'));
        ...
        const setDropdownValue = (dropdown, value) => {
            ...
            const text = dropdown.querySelector('[data-payment-dropdown-text]');
        ...
        const closeDropdown = (dropdown) => {
            ...
            if (!document.querySelector('[data-payment-dropdown].is-open')) {
                cashPaymentForm.closest('.form-panel')?.classList.remove('has-open-dropdown');
            }
        };
        ...
        dropdowns.forEach((dropdown) => {
            ...
            button?.addEventListener('click', () => {
                ...
                cashPaymentForm.closest('.form-panel')?.classList.toggle('has-open-dropdown', isOpen);
        ...
        const setCreateMode = () => {
            cashPaymentForm.action = cashPaymentForm.dataset.storeUrl;
        ...
        const setUpdateMode = (button) => {
            cashPaymentForm.action = button.dataset.updateUrl;
        ...
        document.querySelectorAll('.delete-form').forEach((form) => {
            form.addEventListener('submit', (event) => {
                ...
                deletePaymentName.textContent = form.querySelector('[data-delete-payment]')?.dataset.deletePayment || 'this entry';
        ...
        paymentDateInput.addEventListener('change', () => {
            ...
            const url = new URL(cashPaymentListUrl, window.location.origin);
            url.searchParams.set('date', paymentDateInput.value);
        ...
        cashPaymentForm.addEventListener('reset', () => {
            setTimeout(() => {
                setCreateMode();
                setDropdownValue(document.getElementById('creditDropdown'), creditInput.value);
                setDropdownValue(document.getElementById('debitDropdown'), debitInput.value);
        ...
        cashPaymentForm.addEventListener('submit', () => {

...
<head>
    ...
    <title>List Of Products | FuelTracker</title>
    ...
    <style>
        ...
        * {
        ...
        .product-list-workspace.app-shell-with-sidebar {
        ...
        .product-list-workspace.app-shell-with-sidebar.menu-collapsed {
...
<body>
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">List Of Products</div>
    ...
    <div class="app-shell-with-sidebar product-list-workspace" id="dashboardPage">
        @include('partials.fueltracker-menu')

        <main class="product-list-page">
            <div class="list-shell">
                <section class="page-title" aria-labelledby="productListTitle">
                    <div>
                        <p class="eyebrow">Masters</p>
                        <h1 id="productListTitle">List Of Products</h1>
                    </div>
                    <span class="record-count">{{ $products->total() }} {{ $products->total() === 1 ? 'record' : 'records' }}</span>
                </section>
                ...
                <section class="list-panel">
                    ...
                    <div class="toolbar">
                        ...
                        <div class="toolbar-actions">
                            @if ($products->count())
                                <a href="{{ route('product.pdf') }}" class="new-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                                <a href="{{ route('product.excel') }}" class="new-btn" data-themed-export>Excel</a>
                            @endif
                            <a href="{{ route('product') }}" class="new-btn">New Product</a>
                        </div>
                    </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    @foreach ($products as $product)
                                        <tr>
                                            <td>{{ $product->id }}</td>
                                            <td class="text-strong">{{ $product->Product_Name }}</td>
                                            <td>{{ $product->HSN }}</td>
                                            <td>{{ $product->Category }}</td>
                                            <td>{{ number_format($product->GST_per, 2) }}%</td>
                                            <td>{{ number_format($product->Purchase_rate, 2) }}</td>
                                            <td>{{ $product->opening_stock }}</td>
                                            <td>{{ optional($product->created_at)->format('d M Y') ?: '-' }}</td>
                                            <td>
                                                <div class="actions">
                                                    <a href="{{ route('product.edit', $product->id) }}" class="action-btn edit-btn">Edit</a>
                                                    <form class="delete-form" method="POST" action="{{ route('product.destroy', $product->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="action-btn delete-btn" data-delete-product="{{ $product->Product_Name }}">Delete</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    @else
                        <div class="empty-state">No products found{{ $search ? ' for "' . $search . '"' : '' }}.</div>
                    @endif
                    ...
                    <div class="pagination-bar">
                        ...
                        @include('partials.compact-pagination', ['paginator' => $products])
                    </div>
                </section>
            </div>
        </main>
    ...
    <div class="delete-modal" id="deleteConfirmModal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-hidden="true">
        <div class="delete-dialog">
            ...
            <div class="delete-dialog-body">
                <p>
                    Are you sure you want to delete <strong id="deleteProductName">this product</strong>? This action cannot be undone.
                </p>
            </div>
    ...
    <script>
        ...
        document.querySelectorAll('.delete-form').forEach((form) => {
            form.addEventListener('submit', (event) => {
                const button = form.querySelector('[data-delete-product]');
                ...
                deleteProductName.textContent = button?.dataset.deleteProduct || 'this product';

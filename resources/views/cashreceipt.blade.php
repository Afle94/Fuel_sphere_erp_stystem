...
<head>
    ...
    <title>Cash Receipt | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cash-receipt-workspace.app-shell-with-sidebar {
        ...
        .cash-receipt-workspace.app-shell-with-sidebar.menu-collapsed {
...
<body>
    @php
        $cashreceipts = collect($cashreceipts ?? $cashreceipt ?? []);
        ...
        $storeUrl = \Illuminate\Support\Facades\Route::has('cashreceipt.store') ? route('cashreceipt.store') : route('cashreceipt');
        $listUrl = \Illuminate\Support\Facades\Route::has('cashreceipt') ? route('cashreceipt') : url('/cashreceipt');
        $hasUpdateRoute = \Illuminate\Support\Facades\Route::has('cashreceipt.update');
        $hasDestroyRoute = \Illuminate\Support\Facades\Route::has('cashreceipt.destroy');
        $hasPdfRoute = \Illuminate\Support\Facades\Route::has('cashreceipt.pdf');
        $hasExcelRoute = \Illuminate\Support\Facades\Route::has('cashreceipt.excel');
    @endphp
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Cash Receipt</div>
    ...
    <div class="app-shell-with-sidebar cash-receipt-workspace" id="dashboardPage">
        @include('partials.fueltracker-menu')

        <main class="cash-receipt-page">
            <section class="page-title" aria-labelledby="cashReceiptTitle">
                <div>
                    <p class="eyebrow">Transactions</p>
                    <h1 id="cashReceiptTitle">Cash Receipt</h1>
                </div>
                <span class="record-count">{{ $cashreceipts->count() }} {{ $cashreceipts->count() === 1 ? 'entry' : 'entries' }}</span>
            </section>

            <div class="content-grid">
                <section class="panel form-panel" aria-labelledby="cashReceiptEntryTitle">
                    <div class="panel-head">
                        <h2 id="cashReceiptEntryTitle">Cash Receipt Entry</h2>
                    </div>
                    ...
                    <form class="receipt-form" id="cashReceiptForm" method="POST" action="{{ $storeUrl }}" data-store-url="{{ $storeUrl }}" autocomplete="off">
                        @csrf
                        <input type="hidden" name="_method" id="cashReceiptFormMethod" value="PUT" disabled>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="field">
                                ...
                                <div class="party-dropdown" id="creditDropdown" data-receipt-dropdown>
                                    ...
                                    <button type="button" class="party-dropdown-button" aria-haspopup="listbox" aria-expanded="false">
                                        <span class="party-dropdown-text" data-receipt-dropdown-text>{{ $selectedCredit ?: 'Select credit' }}</span>
                            ...
                            <div class="field">
                                ...
                                <div class="party-dropdown" id="debitDropdown" data-receipt-dropdown>
                                    <input type="text" class="party-dropdown-value" id="debit" name="debit" value="{{ $selectedDebit }}" autocomplete="off">
                                    <button type="button" class="party-dropdown-button" aria-haspopup="listbox" aria-expanded="false">
                                        <span class="party-dropdown-text" data-receipt-dropdown-text>{{ $selectedDebit ?: 'Select debit' }}</span>
                                        <span class="party-dropdown-arrow" aria-hidden="true"></span>
                                    </button>
                ...
                <section class="panel list-panel" aria-labelledby="cashReceiptListTitle">
                    <div class="table-toolbar">
                        <div class="toolbar-title" id="cashReceiptListTitle">Cash Receipt List</div>
                        <div class="toolbar-actions">
                            @if ($cashreceipts->isNotEmpty() && ($hasPdfRoute || $hasExcelRoute))
                                <div class="export-actions" aria-label="Cash receipt export actions">
                                    @if ($hasPdfRoute)
                                        <a href="{{ route('cashreceipt.pdf', ['date' => $today]) }}" class="export-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                                    @endif
                                    @if ($hasExcelRoute)
                                        <a href="{{ route('cashreceipt.excel', ['date' => $today]) }}" class="export-btn" data-themed-export>Excel</a>
                                    @endif
                                </div>
                            @endif
                    ...
                    <div class="table-wrap">
                        <table>
                            ...
                            <tbody>
                                            ...
                                            <div class="row-actions">
                                                @if ($hasUpdateRoute)
                                                    <button
                                                        ...
                                                        class="update-btn"
                                                        data-edit-receipt
                                                        data-update-url="{{ route('cashreceipt.update', $receipt) }}"
                                                ...
                                                @if ($hasDestroyRoute)
                                                    <form method="POST" action="{{ route('cashreceipt.destroy', $receipt) }}" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="delete-btn" data-delete-receipt="Slip {{ $receipt->slip_no }}">Delete</button>
                                                    </form>
                                                @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="empty-state">No cash receipt entries found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div class="delete-modal" id="deleteConfirmModal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-hidden="true">
        <div class="delete-dialog">
            ...
            <p class="delete-dialog-body">Are you sure you want to delete <strong id="deleteReceiptName">this entry</strong>? This action cannot be undone.</p>
    ...
    <script>
        ...
        const formMethodInput = document.getElementById('cashReceiptFormMethod');
        ...
        const editButtons = document.querySelectorAll('[data-edit-receipt]');
        const cashReceiptListUrl = @json($listUrl);
        const deleteModal = document.getElementById('deleteConfirmModal');
        const deleteReceiptName = document.getElementById('deleteReceiptName');
        const deleteCancelBtn = document.getElementById('deleteCancelBtn');
        const deleteConfirmBtn = document.getElementById('deleteConfirmBtn');
        const dropdowns = Array.from(document.querySelectorAll('[data-receipt-dropdown]'));
        ...
        const setDropdownValue = (dropdown, value) => {
            ...
            const text = dropdown.querySelector('[data-receipt-dropdown-text]');
        ...
        const closeDropdown = (dropdown) => {
            ...
            if (!document.querySelector('[data-receipt-dropdown].is-open')) {
                cashReceiptForm.closest('.form-panel')?.classList.remove('has-open-dropdown');
            }
        };
        ...
        dropdowns.forEach((dropdown) => {
            ...
            button?.addEventListener('click', () => {
                ...
                cashReceiptForm.closest('.form-panel')?.classList.toggle('has-open-dropdown', isOpen);
        ...
        const setCreateMode = () => {
            cashReceiptForm.action = cashReceiptForm.dataset.storeUrl;
        ...
        const setUpdateMode = (button) => {
            cashReceiptForm.action = button.dataset.updateUrl;
            ...
            closeAllDropdowns();
            cashReceiptForm.scrollIntoView({ behavior: 'smooth', block: 'start' });
            slipNoInput.focus();
        };
        ...
        document.querySelectorAll('.delete-form').forEach((form) => {
            form.addEventListener('submit', (event) => {
                ...
                deleteReceiptName.textContent = form.querySelector('[data-delete-receipt]')?.dataset.deleteReceipt || 'this entry';
        ...
        receiptDateInput.addEventListener('change', () => {
            ...
            const url = new URL(cashReceiptListUrl, window.location.origin);
            url.searchParams.set('date', receiptDateInput.value);
            window.location.href = url.toString();
        });

        cancelEditButton.addEventListener('click', () => {
            cashReceiptForm.reset();
            setTimeout(() => {
                setCreateMode();
        ...
        document.addEventListener('click', (event) => {
            if (!event.target.closest('[data-receipt-dropdown]')) {
                closeAllDropdowns();
            }
        ...
        cashReceiptForm.addEventListener('reset', () => {
            setTimeout(() => {
                setCreateMode();
                setDropdownValue(document.getElementById('creditDropdown'), creditInput.value);
                setDropdownValue(document.getElementById('debitDropdown'), debitInput.value);
        ...
        cashReceiptForm.addEventListener('submit', () => {

...
<head>
    ...
    <title>Day Fuel Sales Register | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .search-form { width:min(100%,850px); display:grid; grid-template-columns:minmax(150px,1fr) 132px 132px 74px 66px 116px; align-items:center; gap:8px; }
        ...
        .pagination-links { display:inline-flex; align-items:center; gap:6px; }
    ...
    <style>
        ...
        html[data-theme] .dayfuel-register-page .entries-toggle:hover,
        html[data-theme] .dayfuel-register-page .entries-toggle:focus {
        ...
        html[data-theme] .dayfuel-register-page .entries-dropdown::after {
            border-top-color: var(--primary) !important;
        }

        html[data-theme] .dayfuel-register-page .entries-menu {
            border-color: color-mix(in srgb, var(--primary) 35%, var(--line)) !important;
            box-shadow: 0 18px 38px var(--theme-glow) !important;
        }

        html[data-theme] .dayfuel-register-page .entries-option:hover,
        html[data-theme] .dayfuel-register-page .entries-option:focus,
        html[data-theme] .dayfuel-register-page .entries-option.is-selected {
...
<body>
    @php
        $columns = [
            ...
            'date' => 'Date',
            'items' => 'Item',
            'open' => 'Opening Reading',
            'close' => 'Closing Reading',
            'Test' => 'Test',
            'Quantity' => 'Quantity',
            'rate' => 'Rate',
            'Amount' => 'Amount',
        ...
        $sortUrl = function (string $column) use ($sort, $direction, $search, $perPage) {
            return route('dayfuelregisterfilter', [
                'search' => $search,
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Day Fuel Sales Register</div>
    ...
    <div class="app-shell-with-sidebar dayfuel-register-workspace" id="dashboardPage">
        ...
        <main class="dayfuel-register-page">
            <div class="list-shell">
                <section class="page-title" aria-labelledby="dayFuelRegisterTitle">
                    <div>
                        <p class="eyebrow">Registers</p>
                        <h1 id="dayFuelRegisterTitle">Day Fuel Sales Register</h1>
                    </div>
                ...
                <section class="list-panel">
                    ...
                    <div class="toolbar">
                        <form class="search-form" method="GET" action="{{ route('dayfuelregisterfilter') }}">
                            ...
                            <a href="{{ route('dayfuelregisterfilter') }}" class="reset-btn">Clear</a>
                        ...
                        <div class="toolbar-actions">
                            <a href="{{ route('dayfuelregister.pdf', request()->query()) }}" class="new-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                            <a href="{{ route('dayfuelregister.excel', request()->query()) }}" class="new-btn" data-themed-export>Excel</a>
                        </div>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        @foreach ($columns as $column => $label)
                                            <th class="{{ in_array($column, ['open', 'close', 'Test', 'Quantity', 'rate', 'Amount'], true) ? 'number-cell' : '' }}">
                                ...
                                <tbody>
                                    @foreach ($entries as $entry)
                                        <tr>
                                            ...
                                            <td>{{ $entry->items ?? optional($entry->Nozzle)->Item ?? '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->open) ? number_format($entry->open, 2) : '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->close) ? number_format($entry->close, 2) : '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->Test) ? number_format($entry->Test, 2) : '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->Quantity) ? number_format($entry->Quantity, 2) : '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->rate) ? number_format($entry->rate, 2) : '-' }}</td>
                                            <td class="number-cell">{{ is_numeric($entry->Amount) ? number_format($entry->Amount, 2) : '-' }}</td>
                                            <td class="text-strong">{{ optional($entry->Nozzle)->Nozzle_Name ?? '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="empty-state">No day fuel records found{{ $search ? ' for "' . $search . '"' : '' }}.</div>
                    @endif

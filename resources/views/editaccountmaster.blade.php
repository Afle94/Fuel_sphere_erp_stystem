...
<head>
    ...
    <title>Accounts Master Update | FuelTracker</title>
...
<body>
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Accounts Master Update</div>
            ...
            <div class="account-content">
                <section class="page-title" aria-labelledby="accountMasterTitle">
                    <div>
                        ...
                        <h1 id="accountMasterTitle">Edit Account Details</h1>
                    </div>
                    <a href="{{ route('accounts.index') }}" class="view-list-btn">View List</a>
                </section>

                <section class="account-shell">
                    <form class="form-panel" id="accountMasterForm" method="POST" action="{{ route('accounts.update', $account->id) }}" autocomplete="off" novalidate>
                    @csrf
                    @method('PUT')
                    <div class="form-grid">
                        @if (session('success'))
                        ...
                        <div class="field full">
                            ...
                            <input type="text" id="particulars" name="particulars" placeholder="Enter account name" maxlength="50" value="{{ old('particulars', $account->account_perticular) }}" data-show-limit data-limit-unit="characters" required>
                        </div>

                        <div class="field">
                            <label for="underGroup">Under Group <span class="required-star">*</span></label>
                            <div class="theme-dropdown" id="underGroupDropdown">
                                <input type="text" class="theme-dropdown-value" id="underGroup" name="under_group" maxlength="255" value="{{ old('under_group', $account->under_group) }}" required>
                                <button type="button" class="theme-dropdown-button" id="underGroupButton" aria-haspopup="listbox" aria-expanded="false">
                                    <span class="theme-dropdown-text" id="underGroupText">{{ old('under_group', $account->under_group ?: 'Select under group') }}</span>
                                ...
                                <ul class="theme-dropdown-menu" role="listbox" aria-label="Under group list">
                                    ...
                                    @forelse (($underGroups ?? collect()) as $underGroup)
                                        <li><button type="button" class="theme-dropdown-option {{ old('under_group', $account->under_group) === $underGroup->group_name ? 'is-selected' : '' }}" data-value="{{ $underGroup->group_name }}" role="option" aria-selected="{{ old('under_group', $account->under_group) === $underGroup->group_name ? 'true' : 'false' }}">{{ $underGroup->group_name }}</button></li>
                                    @empty
                        ...
                        <div class="field">
                            ...
                            <div class="balance-control">
                                <input type="number" id="openingBalance" name="opening_balance" min="0" max="99999999.99" step="0.01" value="{{ old('opening_balance', $account->opening_balance) }}" data-decimal-limit="8" data-show-limit data-limit-text="Maximum 8 digits before decimal and 2 after decimal">
                                <div class="balance-type" id="balanceTypeSelect">
                                    <input type="hidden" name="balance_type" id="balanceTypeValue" value="{{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') }}">
                                    <button type="button" class="balance-type-button" id="balanceTypeButton" aria-haspopup="listbox" aria-expanded="false">
                                        <span id="balanceTypeText">{{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') === 'cr' ? 'Cr' : 'Dr' }}</span>
                                    ...
                                    <ul class="balance-type-menu" role="listbox" aria-label="Opening balance type">
                                        <li><button type="button" class="balance-type-option {{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') === 'dr' ? 'is-selected' : '' }}" data-value="dr" role="option" aria-selected="{{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') === 'dr' ? 'true' : 'false' }}">Dr</button></li>
                                        <li><button type="button" class="balance-type-option {{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') === 'cr' ? 'is-selected' : '' }}" data-value="cr" role="option" aria-selected="{{ old('balance_type', $account->transaction_type === 'Cr.' ? 'cr' : 'dr') === 'cr' ? 'true' : 'false' }}">Cr</button></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="field full">
                            ...
                            <textarea id="postalAddress" name="postal_address" placeholder="Enter full postal address" maxlength="500" data-show-limit data-limit-unit="characters">{{ old('postal_address', $account->address) }}</textarea>
                        </div>
                        ...
                        <div class="field">
                            ...
                            <div class="theme-dropdown" id="stateDropdown">
                                <input type="text" class="theme-dropdown-value" id="state" name="state" value="{{ old('state', $account->state) }}">
                                <button type="button" class="theme-dropdown-button" id="stateButton" aria-haspopup="listbox" aria-expanded="false">
                                    <span class="theme-dropdown-text" id="stateText">{{ old('state', $account->state ?: 'Select state') }}</span>
                                    <span class="theme-dropdown-arrow" aria-hidden="true"></span>
                                </button>
                        ...
                        <div class="field">
                            ...
                            <div class="theme-dropdown" id="cityDropdown">
                                <input type="text" class="theme-dropdown-value" id="location" name="location" value="{{ old('location', $account->city) }}">
                                <button type="button" class="theme-dropdown-button" id="cityButton" aria-haspopup="listbox" aria-expanded="false" disabled>
                                    <span class="theme-dropdown-text" id="cityText">{{ old('location', $account->city ?: 'Select state first') }}</span>
                                    <span class="theme-dropdown-arrow" aria-hidden="true"></span>
                                </button>
                        ...
                        <div class="field">
                            ...
                            <input type="email" id="email" name="email" placeholder="name@example.com" maxlength="255" value="{{ old('email', $account->email) }}" data-show-limit data-limit-unit="characters">
                        </div>

                        <div class="field">
                            <label for="phoneLandline">Phone Land Line</label>
                            <input type="tel" id="phoneLandline" name="phone_landline" placeholder="Landline number" maxlength="10" inputmode="numeric" pattern="[0-9]{10}" value="{{ old('phone_landline', $account->phone_number) }}" data-digits-only data-show-limit data-limit-unit="digits">
                        </div>

                        <div class="field">
                            <label for="mobile">Mobile</label>
                            <input type="tel" id="mobile" name="mobile" placeholder="Mobile number" maxlength="10" inputmode="numeric" pattern="[0-9]{10}" value="{{ old('mobile', $account->mobile_number) }}" data-digits-only data-show-limit data-limit-unit="digits">
                        </div>

                        <div class="field">
                            <label for="gstNo">GST No.</label>
                            <input type="text" id="gstNo" name="gst_no" placeholder="GSTIN" maxlength="15" value="{{ old('gst_no', $account->gst_number) }}" data-alphanumeric-only data-show-limit data-limit-unit="characters">
                        </div>
                    </div>
                    ...
                    <p class="form-note">
                        <strong>Note</strong>
                        Update account details carefully so daily transactions, bills, reports, and account ledgers remain accurate.
                    </p>
                    </form>
                </section>
            </div>
    ...
    <script>
        ...
        accountMasterForm.addEventListener('reset', () => {
            setTimeout(() => {
                ...
                stateText.textContent = initialState || 'Select state';
                stateOptions.forEach((option) => {

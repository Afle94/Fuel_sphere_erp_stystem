...
<head>
    ...
    <title>List Of Nozzles | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .back-link:hover,.logout-btn:hover,.primary-btn:hover { background:rgba(255,255,255,.2); transform:translateY(-1px); }
        .logout-btn { font-family:inherit; }
        .nozzle-list-workspace.app-shell-with-sidebar { width:calc(100vw - 24px); min-height:calc(100vh - 88px); grid-template-columns:300px minmax(0,1fr); margin:12px; border-radius:12px; }
        .nozzle-list-workspace.app-shell-with-sidebar.menu-collapsed { grid-template-columns:64px minmax(0,1fr); }
        .nozzle-list-page { min-width:0; padding:14px; }
        ...
        .search-input:focus { border-color:rgba(15,118,110,.52); background:#fff; box-shadow:0 0 0 4px rgba(15,118,110,.13); }
        ...
        .entries-option:hover,.entries-option:focus,.entries-option.is-selected:hover,.entries-option.is-selected:focus { color:#fff; background:linear-gradient(135deg,var(--primary-dark),var(--primary)); outline:none; }
        .entries-option.is-selected { color:var(--ink); background:#fff; font-weight:700; }
        .entries-toggle:focus { border-color:rgba(15,118,110,.52); box-shadow:0 0 0 4px rgba(15,118,110,.13); outline:none; }
        .entries-toggle:hover { border-color:rgba(15,118,110,.42); background:linear-gradient(135deg,rgba(15,118,110,.12),rgba(20,184,166,.07)), #fff; }
        ...
        .sort-link { display:inline-flex; align-items:center; flex-wrap:nowrap; gap:6px; color:#fff; text-decoration:none; white-space:nowrap; }
        ...
        .delete-btn { color:#b42318; background:#fff1f0; font-family:inherit; }
...
<body>
    @php
        $columns = ['id' => 'Sr.', 'Nozzle_Name' => 'Nozzle Name', 'Item' => 'Item', 'Open_Date' => 'Open Date', 'Close_Date' => 'Close Date'];
        $sortUrl = function ($column) use ($sort, $direction, $search, $perPage) {
            return route('nozzle.list', [
                'search' => $search,
                'sort' => $column,
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">List Of Nozzles</div>
    ...
    <div class="app-shell-with-sidebar nozzle-list-workspace" id="dashboardPage">
        @include('partials.fueltracker-menu')
        <main class="nozzle-list-page">
            <div class="list-shell">
                <section class="page-title" aria-labelledby="nozzleListTitle">
                    <div>
                        <p class="eyebrow">Masters</p>
                        <h1 id="nozzleListTitle">List Of Nozzles</h1>
                    </div>
                    <span class="record-count">{{ $nozzles->total() }} {{ $nozzles->total() === 1 ? 'record' : 'records' }}</span>
                </section>
                ...
                <section class="list-panel">
                    ...
                    <div class="toolbar">
                        <form class="search-form" method="GET" action="{{ route('nozzle.list') }}">
                            <input type="hidden" name="sort" value="{{ $sort }}">
                            <input type="hidden" name="direction" value="{{ $direction }}">
                            <input class="search-input" type="search" name="search" value="{{ $search }}" placeholder="Search nozzle or item">
                            <button type="submit" class="search-btn">Search</button>
                            <a href="{{ route('nozzle.list') }}" class="reset-btn">Clear</a>
                        ...
                        <div class="toolbar-actions">
                            @if ($nozzles->count())
                                <a href="{{ route('nozzle.pdf') }}" class="new-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                                <a href="{{ route('nozzle.excel') }}" class="new-btn" data-themed-export>Excel</a>
                            @endif
                            <a href="{{ route('nozzle') }}" class="new-btn">New Nozzle</a>
                        </div>
                    </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    @foreach ($nozzles as $nozzle)
                                        <tr>
                                            <td>{{ $nozzle->id }}</td>
                                            <td class="text-strong">{{ $nozzle->Nozzle_Name }}</td>
                                            <td>{{ $nozzle->Item }}</td>
                                            <td>{{ optional($nozzle->Open_Date)->format('d M Y') ?: '-' }}</td>
                                            <td>{{ optional($nozzle->Close_Date)->format('d M Y') ?: '-' }}</td>
                                            <td>
                                                <div class="actions">
                                                    <a href="{{ route('nozzle.edit', $nozzle->id) }}" class="action-btn edit-btn">Edit</a>
                                                    <form class="delete-form" method="POST" action="{{ route('nozzle.destroy', $nozzle->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="action-btn delete-btn" data-delete-nozzle="{{ $nozzle->Nozzle_Name }}">Delete</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    @else
                        <div class="empty-state">No nozzles found{{ $search ? ' for "' . $search . '"' : '' }}.</div>
                    @endif

                    <div class="pagination-bar">
                        <div>
                            @if ($nozzles->total())
                                Showing {{ $nozzles->firstItem() }} to {{ $nozzles->lastItem() }} of {{ $nozzles->total() }}
                            @else
                                Showing 0 records
                            @endif
                        </div>
                        @include('partials.compact-pagination', ['paginator' => $nozzles])
                    </div>
                </section>
            </div>
        </main>
    ...
    <div class="delete-modal" id="deleteConfirmModal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-hidden="true">
        <div class="delete-dialog">
            ...
            <div class="delete-dialog-body">
                <p>Are you sure you want to delete <strong id="deleteNozzleName">this nozzle</strong>? This action cannot be undone.</p>
            </div>
    ...
    <script>
        ...
        const deleteNozzleName = document.getElementById('deleteNozzleName');
        ...
        document.querySelectorAll('.delete-form').forEach((form) => {
            form.addEventListener('submit', (event) => {
                const button = form.querySelector('[data-delete-nozzle]');
                if (form.dataset.confirmed === 'true') {
                    return;
                }
                event.preventDefault();
                pendingDeleteForm = form;
                deleteNozzleName.textContent = button?.dataset.deleteNozzle || 'this nozzle';

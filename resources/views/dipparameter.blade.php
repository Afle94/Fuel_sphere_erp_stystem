...
<head>
    ...
    <title>Dip Parameter | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .dip-filter-form { min-width:0; flex:1 1 auto; display:flex; align-items:center; gap:8px; }
        .dip-filter-form .fuel-dropdown { width:190px; }
        .dip-search-input { width:min(100%,210px); min-height:31px; padding:0 12px; border:1px solid var(--line); border-radius:8px; color:var(--ink); background:#fbfcfe; font:inherit; font-size:11px; outline:none; }
        .dip-search-input:focus { border-color:rgba(15,118,110,.52); background:#fff; box-shadow:0 0 0 4px rgba(15,118,110,.13); }
        ...
        col.item-col { width:210px; }
        col.depth-col { width:150px; }
        col.liter-col { width:150px; }
        ...
        @media (max-width:760px) {
            ...
            .dip-filter-form { width:100%; align-items:stretch; flex-direction:column; }
            .dip-filter-form .fuel-dropdown { width:100%; }
            .dip-search-input { width:100%; }
...
<body>
    @php
        $sortUrl = function ($column) use ($selectedItem, $search, $sort, $direction) {
            return route('dipparameter.index', [
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Dip Parameter</div>
            ...
            <div class="list-shell">
                <section class="page-title" aria-labelledby="densityListTitle">
                    <div>
                        ...
                        <h1 id="densityListTitle">Dip Parameter</h1>
                    </div>
                ...
                <div class="density-content-grid">
                    <section class="list-panel">
                        <div class="toolbar">
                            <form class="dip-filter-form" method="GET" action="{{ route('dipparameter.index') }}">
                                <label class="filter-label" for="item">Item</label>
                                <div class="fuel-dropdown" data-fuel-dropdown data-submit-on-change>
                                    <input class="fuel-dropdown-input" id="item" name="item" type="hidden" value="{{ $selectedItem }}">
                                    <button class="fuel-dropdown-toggle" type="button" aria-haspopup="listbox" aria-expanded="false">
                                        <span class="fuel-dropdown-text" data-fuel-dropdown-text>{{ $selectedItem ?: 'All Items' }}</span>
                                    ...
                                    <div class="fuel-dropdown-menu" role="listbox">
                                        <button class="fuel-option {{ $selectedItem === '' ? 'is-selected' : '' }}" type="button" role="option" data-value="">All Items</button>
                                        @foreach ($items as $item)
                                            <button class="fuel-option {{ $selectedItem === $item ? 'is-selected' : '' }}" type="button" role="option" data-value="{{ $item }}">{{ $item }}</button>
                                        @endforeach
                                    </div>
                                </div>
                                <input type="hidden" name="sort" value="{{ $sort }}">
                                <input type="hidden" name="direction" value="{{ $direction }}">
                                <input class="dip-search-input" type="search" name="search" value="{{ $search }}" placeholder="Search item, depth, liter" aria-label="Search dip parameter">
                                <button type="submit" class="search-btn">Search</button>
                                <a href="{{ route('dipparameter.index') }}" class="reset-btn">Clear</a>
                            </form>
                            <form class="toolbar-delete-form" method="POST" action="{{ route('dipparameter.destroy-all') }}" data-delete-all-dipparameter data-record-count="{{ $totalDipparameterRecords }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="danger-btn" {{ $totalDipparameterRecords === 0 ? 'disabled' : '' }}>Delete All</button>
                            </form>
                        </div>
                        ...
                        <table class="density-table">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        <a class="sort-link {{ $sort === 'item' ? 'is-active' : '' }}" href="{{ $sortUrl('item') }}">
                                            Item
                                            <span class="sort-mark {{ $sortMark('item') }}" aria-hidden="true"></span>
                                        </a>
                                    </th>
                                    <th class="number-cell">
                                        <a class="sort-link {{ $sort === 'depth' ? 'is-active' : '' }}" href="{{ $sortUrl('depth') }}">
                                            Depth
                                            <span class="sort-mark {{ $sortMark('depth') }}" aria-hidden="true"></span>
                                        </a>
                                    </th>
                                    <th class="number-cell">
                                        <a class="sort-link {{ $sort === 'liter' ? 'is-active' : '' }}" href="{{ $sortUrl('liter') }}">
                                            Liter
                                            <span class="sort-mark {{ $sortMark('liter') }}" aria-hidden="true"></span>
                                        </a>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @if ($data->count())
                                    @foreach ($data as $dipparameter)
                                        <tr>
                                            <td class="number-cell text-strong">{{ $data->firstItem() + $loop->index }}</td>
                                            <td class="text-strong">{{ $dipparameter->item }}</td>
                                            <td class="number-cell">{{ $dipparameter->depth }}</td>
                                            <td class="number-cell">{{ $dipparameter->liter }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td class="empty-state" colspan="4">
                                            No dip parameter records found.
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
                    ...
                    <aside class="import-side" aria-label="Dip parameter import">
                        <section class="import-panel">
                            <h2 class="panel-title">Import Dip Parameter</h2>
                            <form method="POST" action="{{ route('dipparameter.import.store') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="field">
                                    <label for="dipparameter_file">Excel / CSV File</label>
                                    <input class="file-control" id="dipparameter_file" name="dipparameter_file" type="file" required>
                                    <label class="upload-control" for="dipparameter_file">
                                        <span class="upload-info">
                                            ...
                                            <span class="upload-copy">
                                                <span class="upload-title">Choose dip parameter file</span>
                                                <span class="upload-filename" id="dipparameterFileName">No file selected</span>
                                            </span>
                                        </span>
                                        <span class="upload-button">Browse</span>
                                    </label>
                                </div>
                                <p class="help-text">Duplicate rows with the same Item and Depth will be updated with the new Liter value.</p>
                        ...
                        <section class="format-panel" aria-labelledby="formatTitle">
                            <table class="format-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Depth</td>
                                        <td class="sample-value">10</td>
                                    </tr>
                                    <tr>
                                        <td>Liter</td>
                                        <td class="sample-value">120</td>
                                    </tr>
                                </tbody>
                            </table>
                        </section>
    ...
    <div class="delete-modal" id="deleteConfirmModal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-hidden="true">
        <div class="delete-dialog">
            ...
            <div class="delete-dialog-body">
                <p>
                    Are you sure you want to delete <strong id="deleteDipparameterName">all dip parameter records</strong>? This action cannot be undone.
                </p>
            </div>
    ...
    <script>
        document.querySelectorAll('[data-fuel-dropdown]').forEach((dropdown) => {
            ...
            options.forEach((option) => {
                option.addEventListener('click', () => {
                    ...
                    if (dropdown.hasAttribute('data-submit-on-change')) {
                        input.form?.submit();
                    }
                });
            });
        ...
        const dipparameterFileInput = document.getElementById('dipparameter_file');
        const dipparameterFileName = document.getElementById('dipparameterFileName');

        if (dipparameterFileInput && dipparameterFileName) {
            dipparameterFileInput.addEventListener('change', () => {
                dipparameterFileName.textContent = dipparameterFileInput.files.length
                    ? dipparameterFileInput.files[0].name
                    : 'No file selected';
            });
        }

        const deleteModal = document.getElementById('deleteConfirmModal');
        const deleteDipparameterName = document.getElementById('deleteDipparameterName');
        const deleteCancelBtn = document.getElementById('deleteCancelBtn');
        const deleteConfirmBtn = document.getElementById('deleteConfirmBtn');
        const deleteAllDipparameterForm = document.querySelector('[data-delete-all-dipparameter]');
        ...
        if (deleteAllDipparameterForm) {
            deleteAllDipparameterForm.addEventListener('submit', (event) => {
                if (deleteAllDipparameterForm.dataset.confirmed === 'true') {
                    return;
                }

                event.preventDefault();
                pendingDeleteForm = deleteAllDipparameterForm;
                deleteDipparameterName.textContent = `${deleteAllDipparameterForm.dataset.recordCount || 'all'} dip parameter records`;

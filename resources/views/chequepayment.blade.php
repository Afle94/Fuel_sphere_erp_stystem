...
<head>
    ...
    <title>Cheque Payment | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cheque-payment-workspace.app-shell-with-sidebar {
        ...
        .cheque-payment-workspace.app-shell-with-sidebar.menu-collapsed {
...
<body>
    @php
        $chequepayments = collect($chequepayments ?? $chequepayment ?? []);
        ...
        $storeUrl = \Illuminate\Support\Facades\Route::has('chequepayment.store') ? route('chequepayment.store') : route('chequepayment');
        $listUrl = \Illuminate\Support\Facades\Route::has('chequepayment') ? route('chequepayment') : url('/chequepayment');
        $hasUpdateRoute = \Illuminate\Support\Facades\Route::has('chequepayment.update');
        $hasDestroyRoute = \Illuminate\Support\Facades\Route::has('chequepayment.destroy');
        $hasPdfRoute = \Illuminate\Support\Facades\Route::has('chequepayment.pdf');
        $hasExcelRoute = \Illuminate\Support\Facades\Route::has('chequepayment.excel');
    @endphp
    ...
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Cheque Payment</div>
    ...
    <div class="app-shell-with-sidebar cheque-payment-workspace" id="dashboardPage">
        @include('partials.fueltracker-menu')

        <main class="cheque-payment-page">
            <section class="page-title" aria-labelledby="chequepaymentTitle">
                <div>
                    <p class="eyebrow">Transactions</p>
                    <h1 id="chequepaymentTitle">Cheque Payment</h1>
                </div>
                <span class="record-count">{{ $chequepayments->count() }} {{ $chequepayments->count() === 1 ? 'entry' : 'entries' }}</span>
            </section>

            <div class="content-grid">
                <section class="panel form-panel" aria-labelledby="chequepaymentEntryTitle">
                    <div class="panel-head">
                        <h2 id="chequepaymentEntryTitle">Cheque Payment Entry</h2>
                    </div>
                    ...
                    <form class="cheque-form" id="chequepaymentForm" method="POST" action="{{ $storeUrl }}" data-store-url="{{ $storeUrl }}" autocomplete="off">
                        @csrf
                        <input type="hidden" name="_method" id="chequepaymentFormMethod" value="PUT" disabled>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="field">
                                ...
                                <input id="chequeDate" type="date" name="cheque_date" value="{{ old('cheque_date') }}">
                            </div>
                ...
                <section class="panel list-panel" aria-labelledby="chequepaymentListTitle">
                    <div class="table-toolbar">
                        <div class="toolbar-title" id="chequepaymentListTitle">Cheque Payment List</div>
                        <div class="toolbar-actions">
                            @if ($chequepayments->isNotEmpty() && ($hasPdfRoute || $hasExcelRoute))
                                <div class="export-actions" aria-label="Cheque Payment export actions">
                                    @if ($hasPdfRoute)
                                        <a href="{{ route('chequepayment.pdf', ['date' => $today]) }}" class="export-btn" target="_blank" rel="noopener" data-themed-export>PDF</a>
                                    @endif
                                    @if ($hasExcelRoute)
                                        <a href="{{ route('chequepayment.excel', ['date' => $today]) }}" class="export-btn" data-themed-export>Excel</a>
                                    @endif
                                </div>
                            @endif
                            ...
                            <tbody>
                                @forelse ($chequepayments as $payment)
                                    <tr>
                                        ...
                                        <td>{{ optional($payment->cheque_date)->format('Y-m-d') }}</td>
                                            ...
                                            <div class="row-actions">
                                                @if ($hasUpdateRoute)
                                                    <button
                                                        ...
                                                        class="update-btn"
                                                        data-edit-cheque
                                                        data-update-url="{{ route('chequepayment.update', $payment) }}"
                                                        data-date="{{ optional($payment->date)->format('Y-m-d') }}"
                                                        data-slip-no="{{ $payment->slip_no }}"
                                                        data-cheque-no="{{ $payment->cheque_no }}"
                                                        data-cheque-date="{{ optional($payment->cheque_date)->format('Y-m-d') }}"
                                                ...
                                                @if ($hasDestroyRoute)
                                                    <form method="POST" action="{{ route('chequepayment.destroy', $payment) }}" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="delete-btn" data-delete-cheque="Slip {{ $payment->slip_no }}">Delete</button>
                                                    </form>
                                                @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="empty-state">No Cheque Payment entries found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <script>
        const chequepaymentForm = document.getElementById('chequepaymentForm');
        const formMethodInput = document.getElementById('chequepaymentFormMethod');
        ...
        const chequepaymentListUrl = @json($listUrl);
        ...
        const closeDropdown = (dropdown) => {
            ...
            if (!document.querySelector('[data-cheque-dropdown].is-open')) {
                chequepaymentForm.closest('.form-panel')?.classList.remove('has-open-dropdown');
            }
        };
        ...
        dropdowns.forEach((dropdown) => {
            ...
            button?.addEventListener('click', () => {
                ...
                chequepaymentForm.closest('.form-panel')?.classList.toggle('has-open-dropdown', isOpen);
        ...
        const setCreateMode = () => {
            chequepaymentForm.action = chequepaymentForm.dataset.storeUrl;
        ...
        const setUpdateMode = (button) => {
            chequepaymentForm.action = button.dataset.updateUrl;
            ...
            closeAllDropdowns();
            chequepaymentForm.scrollIntoView({ behavior: 'smooth', block: 'start' });
            slipNoInput.focus();
        };
        ...
        paymentDateInput.addEventListener('change', () => {
            ...
            const url = new URL(chequepaymentListUrl, window.location.origin);
            url.searchParams.set('date', paymentDateInput.value);
            window.location.href = url.toString();
        });

        cancelEditButton.addEventListener('click', () => {
            chequepaymentForm.reset();
            setTimeout(() => {
                setCreateMode();
        ...
        chequepaymentForm.addEventListener('reset', () => {
            setTimeout(() => {
                setCreateMode();
                setDropdownValue(document.getElementById('creditDropdown'), creditInput.value);
                setDropdownValue(document.getElementById('debitDropdown'), debitInput.value);
        ...
        chequepaymentForm.addEventListener('submit', () => {

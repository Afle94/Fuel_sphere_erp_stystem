...
<head>
    <meta charset="UTF-8">
    <title>Cheque Receipt List</title>
...
<body>
    <div class="report-header">
        <h1>Cheque Receipt List</h1>
        <div class="report-meta">
            Date: {{ $selectedDate }} | Generated on {{ now()->format('d M Y h:i A') }} | Total Entries: {{ $chequereceipts->count() }}
        </div>
    </div>

    @if ($chequereceipts->count())
        ...
        <table>
            ...
            <tbody>
                @foreach ($chequereceipts as $receipt)
                    <tr>

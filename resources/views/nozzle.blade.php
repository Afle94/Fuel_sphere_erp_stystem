...
<head>
    ...
    <title>Nozzle Names Maintenance | FuelTracker</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .site-logo-icon { display:grid; width:34px; height:34px; place-items:center; border-radius:12px; color:var(--primary); background:#fff; box-shadow:0 10px 28px rgba(0,0,0,.18); overflow:hidden; padding:2px; }
        ...
        .back-link,.logout-btn,.save-btn { display:inline-flex; align-items:center; justify-content:center; min-height:36px; border-radius:10px; cursor:pointer; font-size:14px; font-weight:700; text-decoration:none; transition:background .2s ease, color .2s ease, transform .2s ease, box-shadow .2s ease; }
        ...
        .nozzle-page { width:min(100% - 36px,900px); display:block; margin:0 auto; padding:14px 0 70px; transform:translateX(86px); transition:transform .22s ease; }
        ...
        .view-list-btn { flex:0 0 auto; min-height:34px; display:inline-flex; align-items:center; justify-content:center; padding:0 15px; border:1px solid rgba(15,118,110,.2); border-radius:12px; color:var(--primary-dark); background:rgba(15,118,110,.08); font-size:13px; font-weight:700; text-decoration:none; }
        ...
        .form-actions { display:flex; align-items:center; justify-content:flex-end; gap:10px; margin-top:16px; padding-top:14px; border-top:1px solid var(--line); }
        .save-btn { min-width:118px; border:0; color:#fff; background:linear-gradient(135deg,var(--primary-dark),var(--primary)); box-shadow:0 12px 24px rgba(15,118,110,.22); }
        .save-btn:disabled { cursor:not-allowed; opacity:.7; }
        .clear-btn { min-height:36px; padding:0 18px; border:1px solid var(--line); border-radius:10px; color:var(--muted); background:#fff; cursor:pointer; font-size:14px; font-weight:700; }
        .form-note { margin:14px 0 0; color:var(--muted); font-size:13px; line-height:1.5; }
        @media (max-width:940px) { .nozzle-page { transform:none; padding:18px; } }
        @media (max-width:640px) { .site-header-inner{min-height:54px;padding:0 10px}.site-logo{font-size:20px}.header-actions{right:10px;gap:6px}.header-title{font-size:14px}.back-link,.logout-btn{min-height:32px;padding:0 7px;font-size:12px}.page-title h1{font-size:19px}.eyebrow{display:none}.form-panel{padding:10px;border-radius:16px}.form-grid{grid-template-columns:1fr}.field label,.field input,.field select,.theme-select-toggle{font-size:13px}.field input,.field select,.theme-select-toggle{min-height:36px;padding:0 9px}.field select{padding-right:36px;background-position:right 10px center}.save-btn,.clear-btn{min-height:32px;font-size:12px} }
    </style>
...
<body>
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <div class="header-title">Nozzle Names Maintenance</div>
    ...
    <div class="nozzle-workspace account-master-workspace" id="dashboardPage">
        ...
        <main class="nozzle-page">
            <section class="page-title" aria-labelledby="nozzleMasterTitle">
                <div>
                    <p class="eyebrow">Masters</p>
                    <h1 id="nozzleMasterTitle">Nozzle Details</h1>
                </div>
                <a href="{{ route('nozzle.list') }}" class="view-list-btn">View List</a>
            </section>

            <form class="panel form-panel" id="nozzleForm" method="POST" action="{{ route('nozzle.create') }}" autocomplete="off">
                @csrf
                ...
                <div class="form-grid">
                    <div class="field">
                        ...
                        <input type="text" id="Nozzle_Name" name="Nozzle_Name" maxlength="50" value="{{ old('Nozzle_Name') }}" placeholder="Enter nozzle name" required>
                    </div>
                    @php
                        $selectedItem = old('Item');
                    @endphp
                    <div class="field">
                ...
                <p class="form-note"><strong>Note</strong> Maintain each nozzle name with its linked item and active dates for fuel sales and reports.</p>
            </form>
        </main>
    </div>
